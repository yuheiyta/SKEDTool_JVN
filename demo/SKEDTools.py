...
class DRG:
        ...
        for sked in self.sked.skeds:
            ...
            for sked_antenna in sked.antennas:
                ...
                if(altaz_i.alt.deg < float(sked_antenna.lim[1][0]) or float(sked_antenna.lim[1][1]) < altaz_i.alt.deg):
                    print("antenna elevation limit", sked_source.name, sked.start, sked_antenna.name)
                    numerror+=1
                if(altaz_e.alt.deg < float(sked_antenna.lim[1][0]) or float(sked_antenna.lim[1][1]) < altaz_e.alt.deg):
                    print("antenna elevation limit", sked_source.name, sked.start+dur, sked_antenna.name)
                    numerror+=1
                ...
                if(prevtime!=0):
                    if(np.abs(prevpos[i].az.deg - altaz_i.az.deg)/float(sked_antenna.rate[0]) > (sked.start - prevtime).sec/60.):
                        print("antenna slew error",sked.start, sked_antenna.name)
                        numerror+=1
                ...
                while(False in on_source):
                    ...
                    if(int(time2end.sec) >  int(dt)):
                        for j,antenna in enumerate(sked.antennas):
                            ...
                            az[j],el[j],on_source[j]=ant_slew(az[j],el[j],nextaz,nextel,antenna.rate[0],antenna.rate[1],dt)
                        if(False not in on_source):
                            starttime=start_time+dt_time
                            break
                        obstime=nextobstime
                        for j,antenna in enumerate(sked.antennas):
                            altaz=AltAz(az=str(az[j])+"d",alt=str(el[j])+"d",obstime=obstime,location=antenna.coord)
    ...
    #                         df=df.append({'source':skdlist[i], 'time':obstime.datetime, 'antenna az':az,'antenna el':el,'sun az':sunaz,'sun el':sunel},ignore_index=True)
                        ...
                        for j,antenna in enumerate(sked.antennas):
                            nextaltaz,nextaz,nextel=trans_azel(sked.source.coord,nextobstime,antenna.coord)
                            az[j],el[j],on_source[j]=ant_slew(az[j],el[j],nextaz,nextel,antenna.rate[0],antenna.rate[1],time2end.sec)
                        obstime=nextobstime
                        for j,antenna in enumerate(sked.antennas):
                            altaz=AltAz(az=str(az[j])+"d",alt=str(el[j])+"d",obstime=obstime,location=antenna.coord)
                            sunaltaz,sunaz,sunel=trans_azel(get_sun(obstime),obstime,antenna.coord)
            ...
            if(False not in on_source):
                ...
                while(int(time2end.sec) >= int(dt)):
                    for j,antenna in enumerate(sked.antennas):
                        #add az, el limit
                        altaz,az[j],el[j]=trans_azel(sked.source.coord,obstime,antenna.coord)
                        sunaltaz,sunaz,sunel=trans_azel(get_sun(obstime),obstime,antenna.coord)
                        sunsep=altaz.separation(sunaltaz)
                        if(sunsep.deg<=sunseplim):
                            print("Warning: separation = ","{:.3f}".format(sunsep.deg),"deg","#",i+1,sked.name,obstime,antenna.name)
                ...
                while(int(time2next.sec) > int(dt_time.sec)):
                    ...
                    for j,antenna in enumerate(sked.antennas):
                        ...
                        if(sunsep.deg<=sunseplim_slew):
                            print("Warning: separation = ","{:.3f}".format(sunsep.deg),"deg","slew from #",i+1,self.sked.skeds[i].name,"to","#",i+2,self.sked.skeds[i+1].name,obstime)
                            #df_warn=df_warn.append({'source':'slew', 'time':obstime.datetime, 'antenna az':az,'antenna el':el,'sun az':sunaz,'sun el':sunel},ignore_index=True)
                    #df=df.append({'source':'slew    ', 'time':obstime.datetime, 'antenna az':az,'antenna el':el,'sun az':sunaz,'sun el':sunel},ignore_index=True)
                        azellist[j].append([obstime.datetime,az[j],el[j]])
                    time2next=self.sked.skeds[i+1].start-obstime
                nextobstime=obstime+time2next
                for j,antenna in enumerate(sked.antennas):
                    nextaltaz,nextaz,nextel=trans_azel(self.sked.skeds[i+1].source.coord,nextobstime,antenna.coord)
                    az[j],el[j],on_source[j]=ant_slew(az[j],el[j],nextaz,nextel,antenna.rate[0],antenna.rate[1],time2next.sec)
                obstime=nextobstime
        return azellist
    ...
    def sourceplot(self,coord="equitorial",showlabel=True):
        self.source.plot(coord,showlabel)
    def el_plot(self,srcnames=[],refant="",timezone="lst",ellim=[],timelim=[]):
        ...
        for src in srcs:
            ...
            for loc_i,location in enumerate(locations):
                ...
                ax.scatter(timelist, el_array,label=location.name)
                j+=1
            # ax.xaxis.set_major_formatter(mdates.DateFormatter("%H:%M"))
            # ax.set_title("Start Time: "+str(start_time.utc))
            ax.set_title(src.name)
            if(timezone=='lst'.casefold()):
                ax.set_xlabel("Time (LST), Reference = "+refantname)
            elif(timezone=='ut'.casefold() or timezone=='utc'.casefold()):
                ax.set_xlabel("Time (UT) in "+obsdate)
            elif(timezone=='jst'.casefold()):
                ax.set_xlabel("Time (JST) in "+obsdate)
            ...
            if(len(ellim)>=1):
                for el in ellim:
                    ax.axhline(y=el,color="black",ls="dashed")
            if(len(timelim)>=1):
                for time in timelim:
                    ax.axvline(x=time,color="black",ls="dotted")
            ax.legend()
            fig.tight_layout()
...
class Source:
    ...
    def __str__(self):
        name1 = self.name1
        name2 = self.name2
        ra = self.coord.ra.hms
        dec = self.coord.dec.dms
...
class SKD_Source:
    ...
    def __str__(self):
        ...
        for source in self.sources:
            name1 = source.name1
            name2 = source.name2
            ra = source.coord.ra.hms
            dec = source.coord.dec.dms
    ...
    def plot(self,coord="equitorial",showlabel=True):
        ...
        for source in self.sources:
            ...
            if("gal".casefold() in coord.casefold()):
                ...
                ax.scatter(source.coord.ra.radian, source.coord.dec.radian, label=source.name)
...
class SKD_Station:
    ...
    def output(self):
        ...
        for antenna in self.stations:
            ...
            outline = "A  {:1s} {:<8.8s} {:4s}   {:.2f}   {:.1f}    {:.1f}    {:.1f}  {:.1f}   {:.1f}    {:.1f}    {:.1f}   {:.1f}   {:.1f} {:2s} {:2s}".format(
...
class SKD_Exper:
    def __init__(self, obscode, name="", correlator="", *args):
...
def ant_slew(az,el,nextaz,nextel,antspeed_az,antspeed_el,dt):
